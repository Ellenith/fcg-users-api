...
public class UsuarioController : ControllerBase
{
    ...
    [EndpointDescription("Gera um token JWT para um usuário cadastrado.")]
    public async Task<IActionResult> Login([FromBody] LoginRequest request)
    {
        ...
        var senhaValida = _passwordHasher.VerificarSenha(request.Senha, usuario.PasswordHash);
        if (!senhaValida)
            return Unauthorized("Credenciais inválidas.");

        var token = _jwtServico.GerarToken(usuario);

        return Ok(new LoginResponse(usuario.Id, usuario.Name, usuario.Email, usuario.Role, token));
    }
    ...
    private Guid ObterUsuarioLogadoId()
    {
        ...
        if (string.IsNullOrWhiteSpace(sub) || !Guid.TryParse(sub, out var usuarioLogadoId))
